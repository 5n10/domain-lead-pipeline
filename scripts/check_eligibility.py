"""Check eligibility and processing status."""
...
with session_scope() as session:
    # Leads with score >= 40 and no website
    stmt = select(func.count(Business.id)).where(
        Business.lead_score >= 40.0, 
    ...
    print(f"Total Eligible Leads (Score>=40, No Website): {total_eligible}")
    print(f"Processed by Domain Guess: {processed_dg}")
    print(f"Processed by DDG: {processed_ddg}")
    print(f"Remaining for Domain Guess: {total_eligible - processed_dg}")
